...
def fetch_and_append_data_merged(interval):
    ...
    csv_file = f'{ticker}_Bybit-Bitget - {interval} Sec.csv'
...
def base(x, y, cancel_counter, order_counter, interval):
    ...
    ask_df = pd.read_csv(f'{ticker}_Bybit-Bitget - {interval} Sec.csv')
    ask_df= ask_df[['timestamp', f'Bybit_{ticker}USDC_ask', f'Bitget_{ticker}USDTUMCBL_ask']]
    ask_df['ask_diff'] = ask_df[f'Bybit_{ticker}USDC_ask'] - ask_df[f'Bitget_{ticker}USDTUMCBL_ask']
    ...
    bid_df = pd.read_csv(f'{ticker}_Bybit-Bitget - {interval} Sec.csv')
    bid_df = bid_df[['timestamp', f'Bybit_{ticker}USDC_bid',  f'Bitget_{ticker}USDTUMCBL_bid']]
    bid_df['bid_diff'] = bid_df[f'Bybit_{ticker}USDC_bid'] - bid_df[f'Bitget_{ticker}USDTUMCBL_bid']
    ...
    def transactions_data(pos, bybit_transaction, bitget_transaction):
        ...
        csv_file = f"Result_{ticker}-Bybit-Bitget - {interval} Sec.csv"
...
###### Bitget Variable ######
ticker = 'XRP'
bitget_symbol = f'{ticker}USDT_UMCBL'
qty = 30
product_type = 'UMCBL'
marginCoin = 'USDT'
...
while True:
    try:
        ...
        tg_pop(f'Bybit / Bitget {ticker} error: {e},\n traceback: {traceback.format_exc()}')
        break
