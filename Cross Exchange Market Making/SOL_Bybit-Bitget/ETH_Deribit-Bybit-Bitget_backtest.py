...
df = pd.read_csv(f'{symbol}_Bybit-Bitget - {interval} Sec.csv')
# print(df)
# time.sleep(1234)
# df = df[['timestamp','Binance_BTCTUSD', 'Bybit_BTCUSDT']]
df = df[[f'timestamp',f'Bybit_{symbol}USDC_bid', f'Bybit_{symbol}USDC_ask' , f'Bitget_{symbol}USDTUMCBL_bid' , f'Bitget_{symbol}USDTUMCBL_ask']]
...
# ASK
# symbol_1 = 'Bybit_XRPUSDC_ask'
# symbol_2 = 'Bitget_XRPUSDTUMCBL_ask'

# BID
symbol_1 = f'Bybit_{symbol}USDC_bid'
symbol_2 = f'Bitget_{symbol}USDTUMCBL_bid'
...
def base(x, y):
    ...
    # df['pnl'] = np.where(df['pos_change'] != 0, df['pos_change'] * (df['diff'] - df['entry_price']), 0)
    df['fee'] = df[symbol_2]*trading_size*0.00002
