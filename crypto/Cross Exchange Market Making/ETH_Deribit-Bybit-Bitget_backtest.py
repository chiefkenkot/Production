...
# ASK
ask_symbol_1 = 'Bybit_ETHUSDC_ask'
ask_symbol_2 = 'Deribit_ETHPERPETUAL_ask'
ask_symbol_3 = 'Bitget_ETHPERPCMCBL_ask'

# BID
bid_symbol_1 = 'Bybit_ETHUSDC_bid'
bid_symbol_2 = 'Deribit_ETHPERPETUAL_bid'
bid_symbol_3 = 'Bitget_ETHPERPCMCBL_bid'

###### Fee Version ######
# def base(x, y):
#     df['diff'] = df[ask_symbol_1] - df[ask_symbol_3]
...
#
#     df['fee'] = df[ask_symbol_3]*0.003*0.00002
#     # df['fee'] = 0
#     # df['fee'] = 0
...
#     df['pos'] = np.where((df['diff'] > (df['ma'] + df['fee']), 1, (np.where(df['diff'] < (df['ma'] - df['fee'], -1, 0)))))
...
def base(x, y):
    # df['diff'] = df[ask_symbol_1] - df[ask_symbol_3]
    df['diff'] = df[bid_symbol_1] - df[bid_symbol_3]
    ...
    # df['pnl'] = np.where(df['pos_change'] != 0, df['pos_change'] * (df['diff'] - df['entry_price']), 0)
    df['fee'] = df[ask_symbol_3]*trading_size*0.00002
...
fig = px.line(df, x='timestamp', y=['cumu'], title=f'Strategy: {ask_symbol_1} vs {ask_symbol_3}, Interval = {interval} sec ')
