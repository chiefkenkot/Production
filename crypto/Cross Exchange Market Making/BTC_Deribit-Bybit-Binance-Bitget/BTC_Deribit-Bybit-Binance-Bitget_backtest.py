...
df = pd.read_csv('BTC_Deribit-Bybit-Binance-Bitget - 10 Sec.csv')
# df = df[['timestamp','Binance_BTCTUSD', 'Bybit_BTCUSDT']]
df = df[['timestamp','Binance_BTCTUSD', 'Bybit_BTCUSDT', 'Deribit_BTCPERPETUAL', 'Bitget_BTCUSDTUMCBL']]
...
##### BBAND Version ######
def base(x, y):
    df['diff'] = (df['Binance_BTCTUSD'] - df['Bitget_BTCUSDTUMCBL'])*0.001
    # df['diff'] = df['Binance_BTCTUSD'] - df['Deribit_BTCPERPETUAL']
    df['ma'] = df['diff'].rolling(x).mean()
    df['sd'] = df['diff'].rolling(x).std()
    df['z'] = (df['diff'] - df['ma']) / df['sd']
    ...
    # df['pnl'] = np.where(df['pos_change'] != 0, df['pos_change'] * (df['diff'] - df['entry_price']), 0)
    df['fee'] = df['Bybit_BTCUSDT']*0.004*0.00006
