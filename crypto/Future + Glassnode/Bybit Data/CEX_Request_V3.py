...
class ByBitRequest:
    ...
    def get_open_orders(self, category, symbol=None):
        ...
        if len(response['result']['list']) > 0:
            active_order_side = response['result']['list'][0]['side']
            active_order_price = response['result']['list'][0]['price']
            active_order_status = response['result']['list'][0]['orderStatus']
            active_orderid = response['result']['list'][0]['orderId']
        else:
            # You can replace None with any default value you want to use when there are no orders
            active_order_side = None
            active_order_price = None
            active_order_status = None
            active_orderid = None

        return response, active_order_side, active_order_price, active_order_status, active_orderid

    # no order
    # {'retCode': 0, 'retMsg': 'OK', 'result': {'nextPageCursor': '', 'category': 'spot', 'list': []}, 'retExtInfo': {},
    #  'time': 1693053855156}

    # have order
    # {'retCode': 0, 'retMsg': 'OK', 'result': {'nextPageCursor': '1495838229102929664', 'category': 'spot', 'list': [
    #     {'orderLinkId': '251b1e50-93f3-4cce-a128-3f1882aa61de', 'orderId': '1495838229102929664', 'blockTradeId': '',
    #      'symbol': 'BTCUSDC', 'price': '27000.00', 'isLeverage': '0', 'positionIdx': 0, 'qty': '0.001000',
    #      'side': 'Sell', 'orderStatus': 'New', 'cancelType': 'UNKNOWN', 'rejectReason': '', 'avgPrice': '0',
    #      'leavesQty': '', 'leavesValue': '', 'cumExecQty': '0', 'cumExecValue': '', 'cumExecFee': '',
    ...
    #      'lastPriceOnCreated': '', 'reduceOnly': False, 'closeOnTrigger': False, 'createdTime': '1693053812574',
    #      'updatedTime': '1693053812579', 'smpType': 'None', 'smpGroup': 0, 'smpOrderId': ''}]}, 'retExtInfo': {},
    #  'time': 1693053817427}
    ...
    # Success = {'retCode': 0, 'retMsg': 'OK', 'result': {'orderId': '1496482199118095104', 'orderLinkId': '1693130579777900'}, 'retExtInfo': {}, 'time': 1693138664678}
    # Failed = {'retCode': 170003, 'retMsg': 'An unknown parameter was sent.', 'result': {}, 'retExtInfo': {}, 'time': 1693139167818}
    ...
    def fetch_bybit_data(self, symbol, category):
    ...
    def get_order_history(self, category, symbol=None, baseCoin=None, settleCoin=None, orderId=None,
        ...
        transaction_status = response['result']['list'][0]['orderStatus']
        transaction_side = response['result']['list'][0]['side']

        return response, transaction_price, transaction_status, transaction_side
    ...
    def get_funding_rate_history(self, category, symbol, start_time=None, end_time=None, limit=200):
        end_point = "/v5/market/funding/history"
        params_format = "category={category}&symbol={symbol}&limit={limit}"

        if start_time:
            params_format += "&startTime={start_time}"
        if end_time:
            params_format += "&endTime={end_time}"

        params = params_format.format(category=category, symbol=symbol, limit=limit, start_time=start_time,
                                      end_time=end_time)
    ...
    def get_open_interest(self, category, symbol, interval_time, start_time=None, end_time=None, limit=50, cursor=None):
        ...
        if end_time:
            params_format += "&endTime={end_time}"
        if cursor:
            params_format += "&cursor={cursor}"

        params = params_format.format(category=category, symbol=symbol, interval_time=interval_time, limit=limit,
...
class BinanceRequest:
    ...
    # exceptions:
    # try:
    #     response, transaction_price = binance_request.place_order(symbol='BTCTUSD', side='SELL', order_type='MARKET',
    #                                                               quantity=qty * 2)  # BTC qty
    #     print(response)
    #     print(transaction_price)
    # except KeyError as e:
    #     if str(e) == "'fills'":
    #         print('Key Error Catached')
    #     else:
    #         print('nth')
